---
title: "ksqlDB"
description: "This tutorial demonstrates how to integrate Upstash Kafka with ksqlDB"
---
...
Implementing a word count example project can be done with both the ksqlDB CLI and
Java client. In both ways, it is going to be done by consecutive streams. The
operations of the process will be as follows:
...
Install Maven on your machine by following
[Maven Installation Guide](https://maven.apache.org/guides/getting-started/maven-in-five-minutes.html)

Run `mvn –version` in a terminal or command prompt to make sure you have Maven
installed
...
Go into the folder where you want to create the project in your terminal or
...
Create the second stream, which reads from source_stream, splits the string into an
...
The word count stream we created above takes input sentences in JSON format
...
Once you send this message to the "input" topic, you can observe the result at
"OUTPUT" topic as follows:
